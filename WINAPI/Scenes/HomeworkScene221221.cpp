...
bool HomeworkScene221221::Ball1221::CollisionToRect(Rect* lane)
{
    if (!IsCollision(lane))
        return false;
    ...
    Vector2 overlap;
    if(!boundary.IsRectCollision(lane, &overlap))
        return false;
    if(overlap.x < radius && overlap.y < radius){
        ...
        Vector2 v = (pos - lane->pos);
        distance.x = -distance.x + v.x;
        distance.y = -distance.y + v.y;
        distance.Normalize();
    }
    else if (overlap.x < overlap.y || overlap.y == lane->size.y) {
        //좌우 충돌
